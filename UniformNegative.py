...
class NWSDataset(Dataset):
    ...
    def __init__(
        ...
        realidx = [np.random.randint(0, self.fake.shape[0] - val) for _ in range(val)]
        ...
        self.fake_idxs = [np.random.randint(0, self.fakedata.shape[0]-1) for _ in range(self.realdata.shape[0])]
...
for i, val in enumerate([0.75**t for t in range(1, 11)]):
    ...
    for epoch in range(0, 100):
        ...
        for realdata, fakedata in dataloader:
            ...
            lossD = realLoss + (1-lambd)*fakeGenLoss + lambd*fakeLoss
